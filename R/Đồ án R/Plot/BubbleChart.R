...
fig <- plot_ly(country, x = ~Confirmed, y = ~Recovered)
fig <- fig %>% add_trace(text = ~Country_Region, type = 'scatter', mode = 'markers',
               color = ~Deaths, colors = 'Reds',
               marker = list(size = ~Deaths/4000, opacity = 0.8), name='Bubble')
